...
Interfaces are collections of [[method signature]]. A type "implements" an interface if it has all of the methods of the given interface defined on it.
...
Second, complete the `sendMessage` function. It should print a message's `message`, which it obtains through the interface method. Notice that your code doesn't need to worry _at all_ about whether a specific message is a `birthdayMessage` or a `sendingReport`!

# reference:
[[method signature in go]]